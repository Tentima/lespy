...
# Класс Airline: Управляет всеми рейсами: список (или словарь) всех рейсов, методы обавления, удаления, поиска и статистики.
...
class Flight:
    ...
    def show_info(self):
        print(f"{self.route} | Эконом: {self.econom} | Бизнес: {self.business} | "                     
              f"Цена: {self.price_econom}/{self.price_business} | Продано: {self.sold}")
...
class Airline:
    ...
    def add_flight(self, flight):
        if flight.route in self.flights:
            print("Такой рейс уже есть.")
        else:
            self.flight[flight.route] = flight
            print(f"Рейс '{flight.rute}' добавлен.")
